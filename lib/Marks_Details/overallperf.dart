...
void main() {
  runApp(MaterialApp(
    home: OverallPerf(recentSem: 'recentSemValue'),
  ));
}

class OverallPerf extends StatefulWidget {
  final String recentSem;

  OverallPerf( {required this.recentSem});

  @override
  _OverallPerfState createState() => _OverallPerfState();
}

class _OverallPerfState extends State<OverallPerf> {
  ...
  Future<void> fetchSemesters() async {
    ...
    if (response.statusCode == 200) {
      ...
      setState(() {
        ...
        if (_semesterList.contains(widget.recentSem)) {
          _selectedSemester = widget.recentSem;
        } else if (_semesterList.isNotEmpty) {
          _selectedSemester = _semesterList[0];
        }
